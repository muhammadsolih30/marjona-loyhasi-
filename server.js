...
if (DATABASE_CONNECTION_STRING.trim() !== "") {
  try {
    ...
    pool.on("error", (err) => {
      console.warn(
        "Neon connection notice:",
        err ? err.message : "idle client closed",
      );
    });
    ...
    `,
      ...
      .then(() => {
        isDbConnected = true;
        console.log(
          '✅ Neon PostgreSQL bazasiga muvaffaqiyatli ulandi va "leads" jadvali tayyor.',
        );
      })
      .catch((err) => {
        console.error(
  ...
  console.log(
    "ℹ️ DATABASE_URL topilmadi. Hozircha mahalliy zaxira (data/leads.json) rejimida ishlanmoqda.",
  );
  console.log(
    "ℹ️ Neon connection string ni .env fayliga kiritsangiz, avtomatik Neon PostgreSQL bazasiga ulanadi.",
  );
}
...
function requireAuth(req, res, next) {
  ...
  if (!token || !verifyToken(token)) {
    return res
      .status(401)
      .json({ success: false, message: "Ruxsat berilmadi. Qaytadan kiring." });
  }
  next();
}
...
app.post("/api/leads", async (req, res) => {
  try {
    ...
    const budget =
      typeof req.body.budget === "string" ? req.body.budget.trim() : "";
    if (!name || !phone) {
      return res.status(400).json({
        success: false,
        message: "Ism va telefon to'ldirilishi shart.",
      });
    }
    if (
      name.length > 255 ||
      ...
      return res
        .status(400)
        .json({ success: false, message: "Kiritilgan ma'lumotlar juda uzun." });
    }
...
app.post("/api/admin/login", (req, res) => {
  ...
  if (!password || password !== ADMIN_PASSWORD) {
    return res
      .status(401)
      .json({ success: false, message: "Noto'g'ri parol!" });
  }
